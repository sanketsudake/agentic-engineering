"""Tools shared by the baseline and candidate agents (vendored from Lab 12).

Deterministic and in-process. Three of them (`lookup_price`, `get_weather`,
`search_docs`) raise `ValueError` on an unknown key. `tasks.py` uses that to
plant the tool failure the eval must catch.
"""
