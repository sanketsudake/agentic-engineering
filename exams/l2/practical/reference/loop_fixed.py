"""The agent under test: a vendored copy of Lab 3's solution tool loop,
adapted to this exam's tasks (the same shape as Lab 8's agents).

`run_task()` builds a `ScriptedModel` from the task's script, runs the
loop against it, and hands back the result. A tool exception is surfaced
to the model as "error: <message>" — the model can only fix what it can
see (Lab 3, Q 1.4).
"""
...
def run_task(task: dict) -> AgentResult:
    """Run one task through the tool loop."""
