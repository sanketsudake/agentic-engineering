"""Lab 8 tasks: 10 questions the good and buggy agents both attempt.

Four tasks never hit a tool failure — `good_agent` and `buggy_agent` behave
identically on those, byte for byte. Six tasks (`*-err-*`) plant one: the
first scripted tool call uses a slightly wrong key and raises. What happens
next depends on whether the model can SEE that failure in the transcript —
exactly the thing the two agents disagree about (Lab 3's error-surfacing
contract; Trace 10).
...
tool call too — the working agent makes it, the buggy one never does.
"""
...
TASKS = [
    # -- 4 clean tasks: no tool ever fails; good and buggy behave identically.
    {
        "id": "calc-1",
